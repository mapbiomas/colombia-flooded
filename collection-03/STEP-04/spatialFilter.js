 ...
 * STEP 04-2: SPATIAL FILTER
 * ==============================================================================
 * @version       1.0
 * @update        March 2026
 * @attribution   MapBiomas Colombia (Fundacion Gaia Amazonas)
 * @description   Applies a spatial filter to remove isolated flooded (6) pixels.
 *                Uses connectedPixelCount to identify groups smaller than
 *                minConnectedPixels, then replaces them with focal_mode
 *                (neighborhood majority). Missing year bands are filled with 27
 *                (non-flooded) before filtering using a remap([0,6],[27,6],27).
 * @inputs        - Join-filtered classification (clasificacion-ft/)
...
// Build full-band image, remap 0→27 for missing pixels, mask to region
...
param.previewYears.forEach(function(year) {
  ...
  Map.addLayer(allBandsImage.select(selector), vis, 'ClasOriginal ' + year, false);
...
Map.add(ui.Label('Flooded Col3 - Spatial Filter - Region ' + param.regionCode, {
