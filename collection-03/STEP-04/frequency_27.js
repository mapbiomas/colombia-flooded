 ...
 * STEP 04-6: FREQUENCY FILTER (CLASS 27)
 * ==============================================================================
 * @version       1.0
 * @update        March 2026
 * @attribution   MapBiomas Colombia (Fundacion Gaia Amazonas)
 * @description   Applies a frequency filter to stabilize the non-flooded class
 *                (27) across the time series. Pixels classified as non-flooded
 *                in more than majorityPercent of included years are permanently
 *                assigned class 27. Supports year exclusions, an optional spatial
 *                area restriction (applyArea), and polygon-based class remapping.
 *                Within the applyArea, the filter result is blended with the
 *                original classification (applyArea pixels keep original values
 *                outside the filtered area).
 * @inputs        - Frequency-filtered class 6 classification (clasificacion-ft/)
...
// Binary reconstruction: extract class 27 from classification within the region.
// Every pixel within region defaults to 27; class 27 pixels are preserved.
...
// Apply spatial area restriction (blend pattern: outside applyArea keeps original)
if (param.applyArea) {
  var applyImage = param.applyArea
    .map(function(item) { return item.set('id', 1); })
    .reduceToImage(['id'], ee.Reducer.first())
    .clipToBoundsAndScale({ geometry: param.applyArea.geometry(), scale: 30 });

  var noApplyImage = ee.Image(0)
    .where(applyImage.unmask().eq(1), 1)
    .eq(0)
    .selfMask();

  var noAppliedArea = original.updateMask(noApplyImage);
  filtered = filtered.blend(noAppliedArea);
}
...
Map.add(ui.Label('Flooded Col3 - Freq Filter Class 27 - Region ' + param.regionCode, {
...
function frequencyFilter(inputImage) {
  ...
  }, 'Frequency of Non-Flooded (' + usedBands + ' bands)', false);
